// src/pages/admin-change-email/ui/AdminChangeEmailPage.tsx
...
export const AdminChangeEmailPage = observer((): ReactElement => {
  ...
  if (store.emailChangePhase === 'code') {
    return <Navigate to="/admin/profile/security/email/confirm" replace />;
  }
      ...
      <div className={styles.container}>
        ...
        <div className={styles.card}>
          ...
          <p className={styles.lead}>
            Шаг 1 из 2
            <br />
            Текущий email: {profile.email}
          </p>

          <div className={`${styles.field} ${styles.rowNewEmail}`}>
            <div className={styles.label}>Новый email</div>
            <input
              className={`${styles.input} ${styles.inputPlain}`}
              type="email"
              value={store.emailChangeNewEmail}
              onChange={(event) => store.setEmailChangeField('newEmail', event.target.value)}
              placeholder="user@example.com"
              autoComplete="email"
            />
          </div>

          <div className={`${styles.field} ${styles.rowPassword}`}>
            <div className={styles.label}>Пароль от аккаунта</div>
          ...
          <button
            ...
            onClick={async () => {
              await store.requestSuperAdminEmailChange();
              if (store.emailChangePhase === 'code') {
                navigate('/admin/profile/security/email/confirm');
              }
            }}
          >
            {store.isRequestingEmailChange ? 'Отправляем...' : 'Отправить код'}
          </button>
        </div>
      </div>
    </div>
