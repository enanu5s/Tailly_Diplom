...
export class SpecialistReviewRepliesStore {
  ...
  async saveReply({ slug, review }: SaveReplyParams): Promise<SpecialistProfile | null> {
    const text = (
      this.draftsByReviewId[review.id] ??
    ...
    if (text.length > 2000) {
      this.errorsByReviewId[review.id] = 'Ответ слишком длинный. Максимум 2000 символов.';
      return null;
    }
    ...
    try {
      ...
      runInAction(() => {
        this.errorsByReviewId[review.id] =
          error instanceof Error ? error.message : 'Не удалось сохранить ответ на отзыв.';
      });
